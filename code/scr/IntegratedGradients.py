    ...
    def __init__(self, model, outchannels=[], verbose=1):
        ...
        if len(self.outchannels) == 0: 
            ...
            if K.backend() == "tensorflow":
                self.outchannels = range(self.model.output.shape[-1]._value)  ## KV #
            elif K.backend() == "theano":
                self.outchannels = range(self.model.output._keras_shape[1])
            if verbose: print("Evaluated output channel (0-based index): ", self.outchannels)
        else:
            if verbose: 
        ...
        for c in self.outchannels:
            ...
            if K.backend() == "tensorflow":
                gradients = self.model.optimizer.get_gradients(self.model.output[..., c], self.model.input) ## KV #
            if K.backend() == "theano":
